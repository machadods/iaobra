...
**IAObras** é uma plataforma de acompanhamento de obras com integração de IA que visa criar um **Simulador de Estado Temporal da Obra**. A visão é registrar em dados, imagens e vídeos tudo que ocorre em uma obra, permitindo no futuro uma experiência imersiva (VR/AR) onde alguém pode "viajar no tempo" e reviver como trabalhos específicos foram realizados. Combina diário de obra digital com análise de IA, arquivo vivo de mídia, e simulação temporal. **Estrutura:** Camadas profissionais (Models → Services → Repositories → UI).
